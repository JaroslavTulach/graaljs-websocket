...
public class CryptoPolyfillTest {

    private Context context;
    private ExecutorService executor;

    public CryptoPolyfillTest() {
    }
    ...
    public void sublte() throws Exception {
        var result = CompletableFuture
                .supplyAsync(() -> context.eval("js", "typeof crypto.subtle"), executor)
                .get();
    ...
    public void cryptoRandomUUID() throws Exception {
        var result = CompletableFuture
                .supplyAsync(() -> context.eval("js", "crypto.randomUUID()"), executor)
                .get();
    ...
    public void cryptoGetRandomValues() throws Exception {
        ...
        Assert.assertTrue(
                Arrays.stream(result.as(int[].class)).anyMatch(i -> i > 0)
        );
    }
